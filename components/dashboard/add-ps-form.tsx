...
export function AddPSForm({ kabupatenOptions, userRole }: AddPSFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSuccess(`Data berhasil ditambahkan dengan ID: ${result.id}`)

      // Reset form but keep sheet open to show success message
      setFormData({
        skema: "HD",
        pemegang_izin: "",
        desa: "",
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Tambah Data Perhutanan Sosial</SheetTitle>
          <SheetDescription>
