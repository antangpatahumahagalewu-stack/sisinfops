#!/usr/bin/env python3
"""
PROGRAM PLANNER - Insert langsung ke database untuk membuat 12 program aksi mitigasi
Untuk 4 project karbon dengan 3 jenis program masing-masing
"""
...
class ProgramPlannerDirect:
    ...
    def get_required_data(self):
        """Get all required data for program creation"""
        print("📊 Mengumpulkan data yang diperlukan...")
        ...
        if ps_list:
            for i, ps in enumerate(ps_list[:3]):
                print(f"      • {ps['pemegang_izin']} - {ps['desa']}")
        ...
        for kelompok, items in aksi_by_kelompok.items():
        ...
        return {
            ...
            "ps_list": ps_list,
            "aksi_list": aksi_list,
            "aksi_by_kelompok": aksi_by_kelompok
        }
    ...
    def create_program(self, program_data, aksi_ids=None):
        ...
        # Database columns: kode_program, nama_program, jenis_program, kategori_hutan, 
        # tujuan, lokasi_spesifik, target, risiko, carbon_project_id, perhutanan_sosial_id,
        # status, total_budget, budget_status
    ...
    def generate_program_templates(self):
        """Generate 12 program templates for 4 projects × 3 types"""
        
        # Get data
        data = self.get_required_data()
        
        if len(data["projects"]) < 4:
            print("❌ Tidak cukup carbon projects (minimal 4)")
            return []
        
        if len(data["ps_list"]) < 12:
            print("❌ Tidak cukup perhutanan sosial (minimal 12)")
            return []
        ...
        for i, project in enumerate(data["projects"][:4]):
            ...
            for j, prog_type in enumerate(program_types):
                ...
                aksi_ids = self.select_aksi_ids(prog_type["name"], data["aksi_by_kelompok"])
                ...
                program_data = {
                    ...
                    "target": self.generate_target(prog_type["name"], project),
                    "risiko": self.generate_risiko(prog_type["name"], project),
                    "carbon_project_id": project["id"],
                    "perhutanan_sosial_id": ps["id"],
                    "status": "draft",
                    "total_budget": self.calculate_budget(prog_type["name"], project),
                    "budget_status": "draft"
                }
                ...
                print(f"   ✅ Program {j+1}: {kode_program}")
                print(f"      • {nama_program}")
                print(f"      • Jenis: {prog_type['jenis']}")
                print(f"      • Aksi: {len(aksi_ids)} item")
                print(f"      • Budget: Rp {program_data['total_budget']:,.0f}")
    ...
    def generate_target(self, program_type, project):
    ...
    def generate_risiko(self, program_type, project):
    ...
    def run(self):
        """Main execution"""
        print("=" * 80)
        print("PROGRAM PLANNER DIRECT - Membuat 12 Program Aksi Mitigasi")
        print("=" * 80)
        
        # Generate program templates
        print("\n📋 STEP 1: Membuat template program...")
        program_templates = self.generate_program_templates()
        
        if not program_templates:
            print("❌ Gagal membuat template program")
            return
        
        print(f"\n✅ Template program siap: {len(program_templates)} program")
        
        # Ask for confirmation
        print("\n📋 STEP 2: Konfirmasi pembuatan program")
        print(f"   Akan dibuat {len(program_templates)} program:")
        print(f"   • 4 Project Carbon × 3 Jenis Program = 12 Program")
        ...
        # Create programs
        print("\n📋 STEP 3: Menyimpan program ke database...")
        created_programs = []
        
        for i, template in enumerate(program_templates):
            print(f"\n   Program {i+1}/{len(program_templates)}: {template['data']['kode_program']}")
            
            program_id = self.create_program(template['data'], template['aksi_ids'])
            
            if program_id:
                created_programs.append({
                    "id": program_id,
                    "kode": template['data']['kode_program'],
                    "nama": template['data']['nama_program'],
                    "project": template['project'],
                    "type": template['type'],
                    "budget": template['data']['total_budget'],
                    "aksi_count": len(template['aksi_ids'])
                })
        ...
        print("=" * 80)
        print(f"\n✅ Berhasil dibuat: {len(created_programs)} dari {len(program_templates)} program")
        ...
        if created_programs:
            ...
            report_data = {
                ...
                "total_programs": len(program_templates),
                "programs": created_programs
            }
            
            report_file = "program_planner_direct_report.json"
